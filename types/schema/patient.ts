...
export const PatientSchema = z.object({
  name: z.string().min(1, 'Name is required.').max(100, 'Name length can\'t exceed 100 characters'),
  phoneNumber: z.array(z.string().optional()).optional(),
  sex: z.enum(['male', 'female', 'other'], {
    errorMap: () => ({ message: "Please select a sex." }),
  }),
  dob: z.union([z.string().min(1, 'Date of birth is required.'), z.date()]),
  age: z.number().min(0, 'Age must be 0 or greater.').max(120, 'Age cannot exceed 120.').optional(),
  ...
  assignedPhc: z.string().min(1, 'Assigned PHC is required.'),
