...
interface PatientFormStore {
  ...
  openDialog: (mode: 'add' | 'edit', patientData?: PatientFormInputs & { id?: string }, trigger?: React.ReactNode, onSuccess?: () => void) => void
  closeDialog: () => void
  reset: () => void
}
...
export const usePatientFormStore = create<PatientFormStore>((set) => ({
  ...
  openDialog: (mode, patientData, trigger, onSuccess) => set({
    isOpen: true,
    mode,
    patientData,
    trigger,
